...
# Create swarm key for private network if it doesn't exist
...
for i in {1..5}; do
  ...
  ipfs config --json Addresses.API '"/ip4/0.0.0.0/tcp/'$((5001 + 100*port_offset))'"'
  ipfs config --json Addresses.Gateway '"/ip4/0.0.0.0/tcp/'$((8080 + 100*port_offset))'"'
